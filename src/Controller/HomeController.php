...
class HomeController extends AbstractController
{
    ...
    {
        $houses = array_filter(self::LISTINGS, fn($listing) => $listing['property'] === 'house');
        $apartments = array_filter(self::LISTINGS, fn($listing) => $listing['property'] === 'apartment');

        $user = $this->getUser();
        $userId = $user ? $user->getId() : null;

        $favorites = array_column(
            array_filter(self::LISTINGS, fn($listing) => !empty($listing['isFavorited'])),
            'id'
        );
